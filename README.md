...
- 案件を選択し、責任者でグループ化できるロードマップでの WBS 登録・表示・編集・削除
- 同一案件内で子・孫以降を再帰的に追加できるタスク階層
...
```text
src/                        React UI
src/lib/wbs.ts              WBS・責任者・設定の SQLite アクセス
src/lib/calendar.ts         祝日・営業日・カレンダー計算
src/features/wbs/           案件選択・責任者別ロードマップ
