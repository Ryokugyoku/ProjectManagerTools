...
const input: MilestoneInput = { projectId: 2, name: " リリース ", description: " 公開日 ", dueDate: "2026-09-01", completed: false };
...
describe("milestone data methods", () => {
  it("maps milestones with their project identity", async () => {
    db.select.mockResolvedValue([{ id: 1, project_id: 2, project_name: "新製品", project_code: "NEW-01", name: "リリース", description: "公開日", due_date: "2026-09-01", completed: 1 }]);
    expect(await listMilestones()).toEqual([{ id: 1, projectId: 2, projectName: "新製品", projectCode: "NEW-01", name: "リリース", description: "公開日", dueDate: "2026-09-01", completed: true }]);
  });
  it("creates, updates, and deletes a milestone", async () => {
    ...
    expect(db.execute).toHaveBeenNthCalledWith(1, expect.stringContaining("INSERT INTO milestones"), [2, "リリース", "公開日", "2026-09-01", 0]);
    expect(db.execute).toHaveBeenNthCalledWith(2, expect.stringContaining("UPDATE milestones"), [2, "リリース", "公開日", "2026-09-01", 1, 4]);
