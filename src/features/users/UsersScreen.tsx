...
const emptyLeave = (userId = 0): UserLeaveInput => ({ userId, date: formatISODate(new Date()), type: "planned", unit: "full_day", reason: "" });

export function UsersScreen({ users, leaves, onChanged, onError }: { users: Assignee[]; leaves: UserLeave[]; onChanged: () => Promise<void>; onError: (value: string | null) => void }) {
  ...
  useEffect(() => {
    if (selectedLeave) setLeaveForm({ userId: selectedLeave.userId, date: selectedLeave.date, type: selectedLeave.type, unit: selectedLeave.unit, reason: selectedLeave.reason });
