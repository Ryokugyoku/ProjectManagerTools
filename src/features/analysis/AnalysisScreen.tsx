...
function CriticalPathChart({ tasks, projectName, today }: { tasks: WbsTask[]; projectName: string; today: string }) {
  ...
  const aheadIds = new Set(analysis.nodes.filter((node) => node.task.finalized && progressHealth(node.task, today) === "ahead").map((node) => node.task.id));
  const aheadAffectedIds = new Set(aheadIds);
  expanded = true;
  while (expanded) {
    expanded = false;
    for (const edge of analysis.edges) {
      const to = analysis.nodes.find((node) => node.task.id === edge.to)?.task;
      if (to?.status !== "completed" && aheadAffectedIds.has(edge.from) && !aheadAffectedIds.has(edge.to)) { aheadAffectedIds.add(edge.to); expanded = true; }
    }
  }
  ...
  const aheadEdges = new Set(analysis.edges.filter((edge) => aheadAffectedIds.has(edge.from) && !affectedIds.has(edge.from) && analysis.nodes.find((node) => node.task.id === edge.to)?.task.status !== "completed").map((edge) => `${edge.from}-${edge.to}`));
  ...
  const selectedState = networkNodeState(selectedNode, delayedIds, affectedIds, aheadAffectedIds, today);
...
function networkNodeState(node: ReturnType<typeof buildDependencyAnalysis>["nodes"][number], delayedIds: Set<number>, affectedIds: Set<number>, aheadAffectedIds: Set<number>, today: string): NetworkNodeState {
  ...
  if (aheadAffectedIds.has(node.task.id)) return { key: "ahead", label: "前倒し影響", className: "ahead-node" };
