...
function App() {
  ...
  const projectMilestones = useMemo(() => milestones.filter((milestone) => milestone.projectId === selectedRoadmapProjectId), [milestones, selectedRoadmapProjectId]);
  ...
  return (
    <div className="app-frame">
        ...
        <main className="wbs-screen">
          <header className="wbs-header">
            ...
            <div className="header-actions">
              ...
              <button className="quiet-button" onClick={() => setMilestoneEditor("new")}>＋ マイルストーン</button>
          ...
          {!loading && <MilestonePanel milestones={projectMilestones} project={selectedRoadmapProject} today={formatISODate(new Date())} onCreate={() => setMilestoneEditor("new")} onEdit={setMilestoneEditor} />}
          ...
          {loading ? <div className="loading-card">WBSを読み込んでいます…</div> : projectTasks.length === 0 ? <section className="wbs-empty"><span>▦</span><h2>このプロジェクトの最初のタスクを追加しましょう</h2><p>責任者と日程を紐づけると、ロードマップ上で予定と進捗をまとめて確認できます。</p><button className="primary-button" onClick={() => openTaskCreate()}>＋ ロードマップにタスクを追加</button></section> : visibleTasks.length === 0 ? <section className="wbs-empty filtered"><span>⌕</span><h2>条件に一致するWBSがありません</h2><p>検索語または絞り込み条件を変更してください。</p>{hasActiveFilters && <button className="quiet-button" onClick={() => setFilters({ query: "", projectId: selectedRoadmapProject.id, assigneeId: "all", status: "all" })}>絞り込みを解除</button>}</section> : <TimelineBoard tasks={visibleTasks} assignees={assignees} projects={projects} groupBy={groupBy} countryCode={settings.countryCode} selectedId={selectedId} onSelect={(task) => setSelectedId(task.id)} onScheduleChange={updateSchedule} />}
      ...
      {milestoneEditor && selectedRoadmapProject && <MilestoneModal milestone={milestoneEditor === "new" ? null : milestoneEditor} project={selectedRoadmapProject} onClose={() => setMilestoneEditor(null)} onSaved={async () => { setMilestoneEditor(null); await refresh(); }} onError={setError} />}
    </div>
  );
}
